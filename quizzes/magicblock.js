...
genBtn.addEventListener("click", async () => {
  const p = getProfile();
  let r = safeJSONParse(localStorage.getItem(MB_KEYS.resMagic), null);
  if (!r) return;

  if (!r.id){
    r.id = makeId("MagicStudent");
    localStorage.setItem(MB_KEYS.resMagic, JSON.stringify(r));
  }

  await drawQuizResultCard(cardCanvas, {
    quizName: "How well do you know MagicBlock?",
    name: p?.name || "Player",
    avatar: p?.avatar || "",
    correct: r.correct,
    total: r.total,
